...
export default {id:'infrabox-discovery',name:'InfraBox discovery',register(api){
  ...
  const definitions=[
    ...
    {name:TOOLS[2],label:'Read discovery reconciliation',method:'result',description:'Read the compact reconciliation report for an explicit attempt (default 1). Omit host for status counts; select device-ID or vm-ID for changed fields, warnings and errors. Use offset/next_offset for more entries. Read NetBox for current infrastructure state. Raw facts are not exposed.',parameters:{...base,properties:{request_id,attempt:{type:'integer',minimum:1},host:{type:'string'},offset:{type:'integer',minimum:0}}}},
  ];
